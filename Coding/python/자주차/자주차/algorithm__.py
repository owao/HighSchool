from jajuchaUtil import *

# set cascade
trafficlight_cascade = cv2.CascadeClassifier('trafficlight_cascade.xml')
#############################################################################


def findTrafficLight(image):
    # ROI 설정
    height, width = image.shape[:2]
    ROI_image = image[:120, :]
    light = False  # Red light
    # 학습 모델을 사용하여 신호등 위치 추정
    ROI_gray = cv2.cvtColor(ROI_image, cv2.COLOR_BGR2GRAY)
    # trafficlight = trafficlight_cascade.detectMultiScale(ROI_gray, 1.3, 5)
    trafficlight = trafficlight_cascade.detectMultiScale(ROI_gray, 1.1, 4)
    cv2.imshow('roi', ROI_gray)

    for (x, y, w, h) in trafficlight:
        # 신호등이 30cm 이상 밖에 있다고 판단하고 그대로 함수 종료
        if w < 35:
            break

        # 신호등이 30cm 이내 거리에 있다고 판단
        ROI = ROI_gray[y:y + h, x:x + w]
        ROI_X = ROI.shape[1]
        ROI_Y = ROI.shape[0]

        light_position = -1

        # 신호등 위치에서 밝기를 기준으로 왼쪽과 오른쪽 중 어느곳에 불이 들어와 있는지 판단
        for i in range(ROI_X):
            for j in range(ROI_Y):
                if ROI[j, i] > 150:  # 밝기 비교
                    light_position = i

        # 오른쪽에 불이 들어와 있다면 green을 리턴 (기본값 False)
        if light_position > ROI_X / 2 + 4:
            light = True

        return (x, y, w, h, light)

    # 발견된 신호등이 없으면 그냥 False 리턴
    # False is red / True is green
    return False


def autoDrive_algorithm(original_img, canny_img, points, lines, LiDAR, prevComm, status, light):
    height, width = canny_img.shape[:2]
    center = int(width / 2) - 2
    debug = True
    # debug = False
    command = prevComm
    result = findTrafficLight(original_img)

    if result != False:
        x = result[0]
        y = result[1]
        w = result[2]
        h = result[3]
        light = result[4]
        cv2.rectangle(original_img, (x, y), (x + w, y + h), (0, 0, 255), 2)

    H1LD = points['H1LD']
    H1RD = points['H1RD']
    H2LD = points['H2LD']
    H2RD = points['H2RD']
    H3LD = points['H3LD']
    H3RD = points['H3RD']
    H1Y = int(height/4)*1
    H2Y = int(height/4)*2
    H3Y = int(height/4)*3

    V1D = points['V1D']
    V2D = points['V2D']
    V3D = points['V3D']
    V4D = points['V4D']
    V5D = points['V5D']
    V6D = points['V6D']
    V7D = points['V7D']
    V1X = int(width/8)*1        # 1 block distance by y axis
    V2X = int(width/8)*2        # 2 blocks distance by y axis
    V3X = int(width/8)*3        # 3 blocks distance by y axis
    V4X = int(width/8)*4        # 4 blocks distance by y axis
    V5X = int(width/8)*5        # 5 blocks distance by y axis
    V6X = int(width/8)*6        # 6 blocks distance by y axis
    V7X = int(width/8)*7        # 7 blocks distance by y axis

    XD1 = int(width/8)*1        # 1 block distance by x axis
    XD2 = int(width/8)*2        # 2 blocks distance by x axis
    XD3 = int(width/8)*3        # 3 blocks distance by x axis

    leftLane, rightLane, endLane = getLane(lines)
    left = right = end = False
    if len(leftLane) != 0:
        left = True
    if len(rightLane) != 0:
        right = True
    if len(endLane) != 0:
        end = True


    if left and right: #왼쪽 차선과 오른쪽 차선이 있으면
        if H2LD != 160 and H2RD != 160: #중앙 맞추기
            if debug : print('center pint calculted by H2 points %d, %d' % (H2LD, H2RD))
            centerPoint_X = (320 - H2LD + H2RD)/2
        elif H3LD != 160 and H3RD != 160:
            if debug : print('center pint calculted by H3 points %d, %d' % (H3LD, H3RD))
            centerPoint_X = (320 - H3LD + H3RD)/2 #############################

        if centerPoint_X > center+5:
            if centerPoint_X > center-20:
                command = 'S1170E'
            else:
                command = 'S1160E'
        elif centerPoint_X < center+5:
            if centerPoint_X > center-20:
                command = 'S1130E'
            else:
                command = 'S1140E'
        else:#직진
            command = 'S1150E'
            status = ONSTRAIGHT

    elif status is ONSTRAIGHT:#직진중
        print('ONSTRAIGHT')
        elif left and right:
            if debug:
                print('case 1,2/ONSTRAIGHT')
            if getLean(rightLane) < 30###################:
                if debug:
                    print('rightLean')
                command = prevComm
            if H2RD != 160 or H2LD != 160:
                if debug: print('centerpoint by H2 points %d, %d'%(H2LD, H2RD))
                centerPoint_X = (320 - H2LD + H2RD)/2
            elif H3RD != 160 or H3LD != 160:
                if debug: print('centerpoint by H3 points %d, %d'%(H3LD, H3RD))
                centerPoint_X = (320 - H2LD + H2RD)-67
            else: return prevComm, status, #light

            if H1RD is -1 or H2RD is -1:
                print('직진 -> 우회전')
                status = ONRIGHT
            elif centerPoint_X > center:
                if centerPoint_X > center+10:
                    command = 'S1170E'
                else:
                    command = 'S1160E'
            elif centerPoint_X < center-10:
                command = 'S1140E'
            else:
                command = 'S1130E'

            if H1LD is -1 or H2LD is -1:
                print('직진 -> 좌회전')
                status = ONLEFT
            elif centerPoint_X < center:
                if centerPoint_X < center-10:
                    command = 'S1130E'
                else:
                    command = 'S1140E'
            elif centerPoint_X > center+10:
                command = 'S1160E'
            else:
                command = 'S1170E'

        elif right and not left and end:
        elif not right and left and end and H3LD > 90:

    elif status is ONLEFT:
        print('ONLEFT')
        if right and not left and end:
            command = 'S1130E'
            if debug:
                print("onleft")#좌
        elif right and left:
            if (H2LD > 65 and H2RD > 65) and (H3LD > 70 and H2RD > 70):
                command = 'S1150E'
                status = ONSTRAIGHT#중앙맞춤
        elif end:
            print("endline")
            command = 'S1130E'
        else:
            command = prevComm

    elif status is ONRIGHT:
        print('ONRIGHT')
        if left and not right and end:
            command = 'S1170E'
            if debug:
                print("onright")
        elif right and left:
            if (H2LD > 65 and H2RD > 65) and (H3RD > 70 and H3LD > 70):
                command = 'S1150E'
                status = ONSTRAIGHT
        elif end:
            print("endline")
            command = 'S1170E'
        else:
            command = prevComm

    elif status is ONCORNER:
        print("oncorner")
        command = 'S1185E'#자주차 기본 트랙에서 S코너는 우회전 크게 해야함
        if right and left:
            if debug:
                print('onright')
            if H3LD > 60 and H3RD > 60:
                if debug:
                    print('right -> straight')
                command = 'S1150E'
                status = ONSTRAIGHT
    else:
        print("KIN'")
        command = 'S0150E'

    return command, status, light
